#!/usr/bin/env python3
"""
macOS Kernel Extensions Visualization Generator
Generates HTML visualization files for kext data analysis.
"""
...
def create_visualization_html(title, data_file, output_file):
    """
    Create HTML visualization file for kext data.
...
<body>
    <div id="header">
        <h1>{title}</h1>
        <div id="controls">
            <input type="text" id="searchBox" placeholder="Kext search...">
            ...
            <button class="secondary" id="physicsBtn" onclick="togglePhysics()">Disable Physics</button>
        </div>
        <div id="stats">
    ...
    <script>
        ...
        function convertToGraphData(kexts) {{
            const nodesMap = new Map();
            const edgeSet = new Set();
            
            // Create nodes
            for (const [bundleId, kext] of Object.entries(kexts)) {{
                const label = kext.kext_name || kext.name || bundleId.split('.').pop();
                const color = getColorForKext(bundleId, label);
            ...
            for (const [bundleId, kext] of Object.entries(kexts)) {{
                // Dependencies
                for (const dep of kext.dependencies || []) {{
                    if (nodesMap.has(dep)) {{
                ...
                // Libraries
                for (const lib of kext.libraries || []) {{
                    if (nodesMap.has(lib)) {{
        ...
        function getColorForKext(bundleId, label) {{
            // Apple system
            if (bundleId.includes('com.apple')) {{
        ...
        function drawNetwork() {{
            ...
            // Auto-disable physics for force layout after a delay
            if (currentLayout !== 'hierarchical' && physicsEnabled) {{
                setTimeout(() => {{
                    if (physicsEnabled) {{
                        togglePhysics();
                    }}
        ...
        function highlightConnectedNodes(selectedNodeId) {{
            ...
            // Find edges directly connected to current node
            ...
            const updatedNodes = currentData.nodes.map(node => {{
                if (connectedNodeIds.has(node.id)) {{
                    // Connected nodes: maintain original color
                    return {{
                        ...node,
                        borderWidth: 3,
                        font: {{ 
            ...
            const updatedEdges = currentData.edges.map(edge => {{
                if (connectedEdgeIds.has(edge.id)) {{
                    ...
                    return {{
                        ...edge,
                        width: 3,
                        color: {{ color: '#00ffff', opacity: 1.0 }},
                        dashes: false
                    }};
                }} else {{
                    // Unconnected edges: dim
                    return {{
                        ...edge,
                        width: 0.5,
        ...
        function updateStats() {{
            // Calculate actual displayed edge count
            if (network) {{
        ...
        function toggleLayout() {{
            ...
            // Maintain physics state for hierarchical layout
            if (currentLayout === 'hierarchical') {{
                physicsEnabled = true;
            }} else {{
                physicsEnabled = true; // Initially enabled
            }}
        ...
        function showInfoPanel(node) {{
            ...
            panel.innerHTML = `
                ...
                <p><strong>Executable:</strong> ${{node.kext.executable || 'N/A'}}</p>
            `;
        ...
        function reduceEdges() {{
            // Get current displayed edges
            const currentData = network.getData();
            const currentEdges = currentData.edges;
            
            // Set threshold to reduce edge count
...
def generate_visualizations():
    """
    Generate HTML visualization files for both VM Apple and Host kext data.
    """
    print("Generating visualization files...")
    
    # Check if data files exist
    vmapple_data = 'vmapple_kext/kexts_data.json'
    host_data = 'host_kext/kexts_data.json'
    
    if not os.path.exists(vmapple_data):
        print(f"Error: {vmapple_data} not found")
        return
    
    if not os.path.exists(host_data):
        print(f"Error: {host_data} not found")
        return
    
    # Generate VM Apple visualization
    create_visualization_html(
        "🍎 VM Apple Kext Dependency Analysis",
        vmapple_data,
        'vmapple_kext/kext_visualization.html'
    )
    
    # Generate Host visualization
    create_visualization_html(
        "🔌 Host Kext Dependency Analysis", 
        host_data,
        'host_kext/kext_visualization.html'
    )
    
    print("Visualization files generated:")
    print("  - vmapple_kext/kext_visualization.html")
    print("  - host_kext/kext_visualization.html")

def main():
    """
    Main function to generate visualization files.
    """
    print("macOS Kernel Extensions Visualization Generator")
    print("=" * 60)
